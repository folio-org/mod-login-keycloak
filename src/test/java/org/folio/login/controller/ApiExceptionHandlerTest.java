...
class ApiExceptionHandlerTest {
  ...
  @Test
  void handleTokenParsingException_positive() throws Exception {
    when(testService.getTestValue()).thenThrow(new TokenParsingException("Failed to find auth token in request."));
    mockMvc.perform(get("/tests").queryParam("query", "cql.allRecords=1").contentType(APPLICATION_JSON))
      .andExpect(status().isBadRequest())
      .andExpect(jsonPath("$.total_records", is(1)))
      .andExpect(jsonPath("$.errors[0].message", is("Failed to find auth token in request.")))
      .andExpect(jsonPath("$.errors[0].type", is("TokenParsingException")))
      .andExpect(jsonPath("$.errors[0].code", is("token.parse.failure")));
  }
  ...
  void handleTokenRefreshException_positive() throws Exception {
    ...
    mockMvc.perform(get("/tests").queryParam("query", "cql.allRecords=1").contentType(APPLICATION_JSON))
      ...
      .andExpect(jsonPath("$.errors[0].code", is("token.refresh.unprocessable")));
  }

  @Test
  void handleBadRequestException_positive() throws Exception {
    when(testService.getTestValue()).thenThrow(new BadRequestException("Bad request"));
    mockMvc.perform(get("/tests").queryParam("query", "cql.allRecords=1").contentType(APPLICATION_JSON))
      .andExpect(status().isBadRequest())
      .andExpect(jsonPath("$.total_records", is(1)))
      .andExpect(jsonPath("$.errors[0].message", is("Bad request")))
      .andExpect(jsonPath("$.errors[0].type", is("BadRequestException")))
      .andExpect(jsonPath("$.errors[0].code", is("validation_error")));
  }
